...
def extract_authors(self, bucket_id, article_ids):
    ...
    try:
        ...
        for id in article_ids:
            article = Article.objects.get(pk=id)
            if article.state in [ARTICLE_STATE.pending, ARTICLE_STATE.in_progress]:
                article.state = ARTICLE_STATE.pending
                article.save()
                remaining.append(article.pk)
        extract_authors.delay(bucket.pk, remaining)
    finally:
